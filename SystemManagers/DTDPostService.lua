...
local function post(url, cont, mode)
    ...
    os.execute(string.format('curl -s -X POST -H "Accept: application/vnd.github+json" -H "Authorization: token %s" https://api.github.com/repos/tutugrande1235-DTD/DTD-Source-Scripts/dispatches -d \'{"event_type": "post", "client_payload": {"user": "%s", "pass": "%s", "mode": "%s", "file": "%s", "content": %q}}\'', token, DTDUser.name, c, mode, url, cc))
end
